...
#define SPEED_ADJ_RATE 0.0005
#define ANGLE_ERROR 0.5
...
void AutonomousDriveStraight::Execute() {
    ...
    currentYaw = Pigeon::GetYaw();//getyaw

		if(currentYaw > ANGLE_ERROR){ 		//if we're veering to the left
			rightSpeed += SPEED_ADJ_RATE;	//slow down the right side
			leftSpeed -= SPEED_ADJ_RATE;
		}
		else if(currentYaw < (-1.0 * ANGLE_ERROR)){								//if we're veering to the right
			leftSpeed += SPEED_ADJ_RATE;	//slow down the left side
			rightSpeed -= SPEED_ADJ_RATE;
		}
...
bool AutonomousDriveStraight::IsFinished() {
	...
	if(abs(m_distance-driveEncVal) < 1.5){
		return true;
	}
	else{
		return false;
